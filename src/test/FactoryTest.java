...
class FactoryTest extends BaseTest {
    ...
    private void addScalarTests(String name) {
        ...
        test.addTest("값(double 혹은 Double)을 지정하여 스칼라를 생성할 수 있다", () -> {
            Scalar scalarCreateByInitValue = Factory.createScalarFromInitialValue(1.0);
            test.assertTrue(true, "예외 없이 스칼라가 생성됨(double)");

            scalarCreateByInitValue = Factory.createScalarFromInitialValue(new Double(1.0));
            test.assertTrue(true, "예외 없이 스칼라가 생성됨(Double)");
        });
    ...
    private void addVectorTests(String name) {
        ...
        test.addTest("지정한 하나의 값을 모든 요소의 값으로 하는 n-차원 벡터를 생성할 수 있다", () -> {
            Vector vectorCreateByInitValue = Factory.createVectorFromInitialValue(10, Factory.createScalarFromInitialValue(1.0));
            test.assertTrue(true, "예외 없이 벡터가 생성됨");
        });

        test.addTest("i 이상 j 미만의 무작위 값을 요소로 하는 n-차원 벡터를 생성할 수 있다", () -> {
            Vector vectorCreateByMinMaxValue = Factory.createVectorFromRandomValue(10, Factory.createScalarFromInitialValue(1.0), Factory.createScalarFromInitialValue(100.0));
            test.assertTrue(true, "예외 없이 벡터가 생성됨");
        });

        test.addTest("1차원 배열로부터 n-차원 벡터를 생성할 수 있다", () -> {
            Scalar[] arr = new Scalar[10];
            Vector vectorCreateByArray = Factory.createVectorFromArray(arr);
            test.assertTrue(true, "예외 없이 벡터가 생성됨");
        });
    }
    ...
    private void addMatrixTests(String name) {
        ...
        test.addTest("지정한 하나의 값을 모든 요소의 값으로 하는 m x n 행렬을 생성할 수 있다", () -> {
            Matrix matrixCreatedByInitValue = Factory.createMatrixFromInitialValue(10, 10, Factory.createScalarFromInitialValue(1.0));
            test.assertTrue(true, "예외 없이 행렬이 생성됨");
        });

        test.addTest("i 이상 j 미만의 무작위 값을 요소로 하는 m x n 행렬을 생성할 수 있다", () -> {
            Matrix matrixCreatedByMinMaxValue = Factory.createMatrixFromRandomValue(10, 10, Factory.createScalarFromInitialValue(1.0), Factory.createScalarFromInitialValue(100.0));
            test.assertTrue(true, "예외 없이 행렬이 생성됨");
        });
        ...
        test.addTest("2차원 배열로부터 m x n 행렬을 생성할 수 있다", () -> {
            Matrix matrixCreatedByArray = Factory.createMatrixFromArray(new Scalar[10][10]);
            test.assertTrue(true, "예외 없이 행렬이 생성됨");
        });

        test.addTest("단위 행렬을 생성할 수 있다", () -> {
            Matrix matrixCreatedByUnit = Factory.createIdentityMatrix(10);
            test.assertTrue(true, "예외 없이 행렬이 생성됨");
        });
    }
}
